...
function eliminarOk() {
	...
	var urlDel = myContextPath +'/conductor/delete';
...
function editar(id) {
	...
	var urlEdit = myContextPath + '/conductor/edit';
	...
	$.ajax({
		...
		success : function(response) {
			...
			$("#id").val(response.id), $("#dni").val(response.dni);
...
function grabar() {
	...
	if (!msg.trim()) {
		...
		var urlSave = myContextPath + '/conductor/saveOrUpdate';
...
function limpiarForm() {
	...
	$('#frmConductor').find('input, label, select, textarea').removeClass(
			'has_error_per');
	$('#id').val('');
	$("#dni").val('');
	$("#nombre").val('');
...
function tablaForm() {
	...
	var urlLoad = myContextPath + '/conductor/lst';
